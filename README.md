...
_Проверка текущего состояния LVM_
...
_Проверка файловой системы (для home)_
...
_Уменьшение домашнего раздела (lv_home)_
...
_Увеличение корневого раздела (lv_root)_
...
- Убедитесь, что никто не использует файлы в /home во время операции
- Если у вас есть открытые файлы в /home, используйте lsof /home для проверки
- Рекомендуется выполнять операции из single-user mode или rescue mode
- Проверьте, что у вас достаточно свободных экстентов в VG: sudo vgdisplay VG453

_Если возникнут ошибки при уменьшении файловой системы, возможно, потребуется:_

- Проверить занятое место: du -sh /home/\*
- Переместить некоторые данные перед уменьшением
- Использовать resize2fs с флагом -M для минимального размера
